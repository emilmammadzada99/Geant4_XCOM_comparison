...
void RunAction::BeginOfRunAction(const G4Run*)
{
  ...
  std::ofstream outFile("g4_cm2_g.txt", std::ios::app);
  ...
  G4double totalSigma2 = sigma2.back(); // son eleman total
  G4double total_cm2_per_g = totalSigma2 / (cm2 / g);

  G4cout << "\n  total cross section per mass : "
       << std::fixed << std::setprecision(6)
       << total_cm2_per_g << " cm2/g";
...
// --- Sonuçları dosyaya da yaz ---
  if (outFile.is_open()) {
    ...
    //outFile << "compCrossSectionPerVolume: ";
    //for (size_t j = 0; j < sigma0.size(); ++j) {
        //outFile << sigma0[j] * cm << " cm^-1\t";
    //}

    //outFile << "\ncross section per volume : ";
    //for (size_t j = 0; j < sigma1.size(); ++j) {
        //outFile << sigma1[j] * cm << " cm^-1\t";
    //}

    //outFile << "\ncross section per mass   : ";
    //for (size_t j = 0; j < sigma2.size(); ++j) {
        //outFile << G4BestUnit(sigma2[j], "Surface/Mass") << "\t";
    //}
    //outFile << "\ntotal cross section per mass : "
           //<< std::fixed << std::setprecision(6)
           //<< total_cm2_per_g << " cm2/g";
